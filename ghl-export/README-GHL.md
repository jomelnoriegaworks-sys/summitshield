...
| `footer-code.html` | Site/Funnel Settings → Tracking Code → **Footer Tracking** |
...
4. **Paste the footer tracking code.** Settings → Tracking Code → **Footer Tracking** → paste `footer-code.html`. This is the GTM `noscript` fallback for container `GTM-TTWXD7PK`; do not paste it into the page block.
5. **Add the page block.** Add one full-width section → set section/row/element **padding to 0 and width to full** → drag in a **Custom JS/HTML** element → paste all of `3-PAGE-BLOCK.html`. This one block adapts to phones, tablets, and desktop—including GHL's narrow mobile editor—and includes a sticky bottom CTA bar on real mobile screens.
...
7. **Publish and test** on a real phone: anchor links (Coverage/FAQ/Contact), the hamburger menu, the FAQ accordion, the form popup (open, submit, close with the X / Escape / backdrop), and the sticky bottom CTA bar.
